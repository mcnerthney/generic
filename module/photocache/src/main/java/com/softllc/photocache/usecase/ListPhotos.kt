...
class ListPhotosUserNotFoond (val param: ListPhotosUseCase.Param ) : Failure.FeatureFailure()
...
@Inject constructor(
    ...
    : UseCase<List<Photo>, ListPhotosUseCase.Param>() {

    override fun run(params: Param): Flow<Either<Failure, List<Photo>>> = channelFlow {
        ...
        if ( user.id.isNullOrBlank() ) {
            send(Either.Left(ListPhotosUserNotFoond(params)))
        }
        else {
           // insertPhoto()
        }
